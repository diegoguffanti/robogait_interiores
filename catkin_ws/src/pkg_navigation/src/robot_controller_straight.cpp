...
//Contructor del nodo------------------------------------------------------------------
public:
	generic() : Node("robot_controller_straight")
	    {
...
private:
		...
		float kp_yaw=3.0;
		float control_yaw=-kp_yaw*yaw;
		cmd_velrobot_msg.angular.z=control_yaw;
		//cout<< "Robot yaw: "<<yaw<<" control_yaw: "<<control_yaw<<endl;
	    ...
	    void topic_callback_spine_bottom(const body_tracker_msgs::msg::SpineBottom::SharedPtr msg)
	      ...
	      if(counter_distances>2){
		      ...
		      cout<< "Control linear: "<<control<<" Error: "<<error_actual<<" Distance person: "<<distance_person<<endl;
		}
	      }
  	    }
