...
#define path			"/home/robogait/pruebas_grabaciones/kinectRecordings/"
#define fileName		"bodyTracking_test"
#define recordedFPS		30
#define deleteExistingFile	false
...
public:
	...
	void Run(){	
		t_star_recording = rclcpp::Clock().now().nanoseconds();
	...
	k4a_record_write_capture(recording_handle, sensor_capture);
		...
		elapsed_time_recording = (t_now - t_star_recording)* pow(10, -9);
		printf("Han transcurrido %.02f secs\n", elapsed_time_recording);
		pelvis_data.tiempo = elapsed_time_recording;
		...
		//Publishing
		RCLCPP_INFO(this->get_logger(), "Publishing pelvis coordinates. Distance to person: Distance(z)->'%.03f'", pelvis_data.joint_position_spine_bottom.z);
	...
	void Init(){
		...
		sprintf(recordingName, "%s%s.mkv", path, fileName);
		if(k4a_record_create(recordingName, device, device_config, &recording_handle) != K4A_RESULT_SUCCEEDED){
		...
		sprintf(processingName, "%s%s", path, fileName);
	}
	...
	void close(){
		...
		FPS = total_frames / elapsed_time_recording;
		printf("Total FPS = %0.2f\n", FPS);
		printf("Recording finished, time to process into csv file\n");
		t_star_processing = rclcpp::Clock().now().nanoseconds();
		processVideo(recordingName, processingName, deleteExistingFile);
		t_end_processing = rclcpp::Clock().now().nanoseconds();
		elapsed_time_processing = (t_end_processing - t_star_processing)* pow(10, -9);
		printf("Finished processing. Took %0.2f seconds\n", elapsed_time_processing);
		printf("\t\t-------Finished-------\n");
	}
	...
	double t_star_recording, t_now, elapsed_time_recording;		//Recording times
	double t_star_processing, t_end_processing, elapsed_time_processing;	//Processing times
	int counter, total_frames;
	double FPS;
	int inicio;
	char recordingName[100], processingName[100];	
};
