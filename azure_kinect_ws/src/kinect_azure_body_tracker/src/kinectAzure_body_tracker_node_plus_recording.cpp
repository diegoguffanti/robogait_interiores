...
public:
	...
	{
		Init();
		publisher_ = this->create_publisher<body_tracker_msgs::msg::Skeleton>("body_tracking", 10);
		Run();
		close();
	}
	
	void Run(){
		//while (rclcpp::ok() && (t_diff < 10.0)){
		while (rclcpp::ok() && counter < 30){
	...
	void body_tracking()
	{
		...
		//tiempo
		skeleton_data.tiempo = elapsed_time;
		...
		RCLCPP_INFO(this->get_logger(), "Publishing skeleton coordinates. Pelvis coodinates are: tiempo -> '%f' || x->'%f' ; y->'%f' ; z->'%f'", skeleton_data.tiempo,  skeleton_data.joint_position_spine_bottom.x, skeleton_data.joint_position_spine_bottom.y, skeleton_data.joint_position_spine_bottom.x);
		publisher_->publish(skeleton_data);
		//if (skeleton_data.tiempo > 10.0)	return;
		counter++;
  	}
	...
	void Init(){		
		...
		if(k4a_record_create("/home/robogait/pruebas_grabaciones/kinectRecording/output_test.mkv", device, device_config, &recording_handle) != K4A_RESULT_SUCCEEDED){
